...
class DeepEMD(nn.Module):
    ...
    def get_emd_distance(self, similarity_map, weight_1, weight_2):
        ...
        for i in range(num_query):
            for j in range(num_proto):
                _, flow = emd_inference_opencv(similarity_map[i, j, :, :], weight_1[i, j, :], weight_2[j, i, :])
    ...
    def get_similiarity_map(self, proto, query):
        ...
        if self.args.metric == 'Dot':
            similarity_map = (proto * query).sum(-1)
            similarity_map = similarity_map / similarity_map.max()
            similarity_map = 1.0 - similarity_map

        if self.args.metric == 'ADM':
            sim_cos = F.cosine_similarity(proto, query, dim=-1)
            sim_cos = 1.0 - sim_cos
            sim_cos = sim_cos / sim_cos.max()
            sim_l2 = (proto - query).pow(2).sum(-1)
            sim_l2 = sim_l2 / sim_l2.max()
            similarity_map = self.args.cosine_rate * sim_cos + (1.0 - self.args.cosine_rate) * sim_l2
